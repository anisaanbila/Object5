...
# =========================
with tab_docs:
    # 1) Wajib berikan 'options' ke selectbox (tanpa titik-tiga!)
    model_choice = st.selectbox(
        "Pilih jenis model yang ingin dijelaskan:",
        ["Model Deteksi", "Model Klasifikasi"],  # <- ini opsinya
        index=0
    )

    # 2) CSS KHUSUS TAB INI — pastikan triple quote ditutup di baris yang sama blok ini
    st.markdown("""
    <style>
    ...
    # 4) Intro di luar card (berubah sesuai pilihan)
    if model_choice == "Model Deteksi":
        st.markdown(
            """
            <div class="model-intro">
              <b>Ringkasnya:</b> model deteksi (YOLOv8) mencari <i>posisi</i> tangan dan memberi bounding box+label
              dalam satu kali proses (<i>single shot</i>) — cepat untuk real-time.
            </div>
            """, unsafe_allow_html=True
        )
    else:
        st.markdown(
            """
            <div class="model-intro">
              <b>Ringkasnya:</b> model klasifikasi (CNN) fokus pada isi gambar (crop tangan) dan menentukan kelas:
              batu, gunting, atau kertas — sederhana & efisien.
            </div>
            """, unsafe_allow_html=True
        )

    # 5) ====== TEMPATKAN renderer kamu di SINI ======
    # def render_cards_deteksi(): 
    #     ...  (pindahkan fungsi yang sudah kamu buat)
    #
    # def render_cards_klasifikasi():
    #     ...  (pindahkan fungsi yang sudah kamu buat)
    # ================================================

    # 6) Tampilkan sesuai dropdown
    if model_choice == "Model Deteksi":
